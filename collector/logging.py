...
def _load_default_log_config() -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"Log config file not found: {config_path}")
    except orjson.JSONDecodeError as e:
        raise RuntimeError(f"Invalid JSON in log config file {config_path}: {e}")
...
class BaseFormatter(logging.Formatter):
    def __init__(self, service_name: str, version: str, datefmt: str = None):
...
def _create_formatter(log_format: str, service_name: str, version: str) -> logging.Formatter:
    formatters = {
